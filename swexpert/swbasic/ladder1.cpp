...
const int MAX = 100;
int dir[3][2] = { {0,-1},{0,1},{-1,0} };//왼,오,위
int x, y;
int BFS(int map[MAX+2][MAX+2],bool visited[MAX+2][MAX+2]);
int main()
{
...
int BFS(int map[MAX + 2][MAX + 2], bool visited[MAX + 2][MAX + 2])
{
	...
	while (!q.empty()) {
		...
		for (int i = 0; i < 3; i++) {
			int xxx = xx + dir[i][1];
			int yyy = yy + dir[i][0];
			if (!visited[yyy][xxx] && map[yyy][xxx] == 1) {
				pos = make_pair(xxx, yyy);
				q.push(pos);
