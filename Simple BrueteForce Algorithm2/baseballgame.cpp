...
int main()
{
    ...
    for (int tc = 0; tc < n; tc++)
    {
        cin >> num >> s >> b;
        baseball[0] = num / 100;
        baseball[1] = (num - baseball[0] * 100) / 10;
        baseball[2] = (num % 100) % 10;
        ...
        if (tc == 0)
        {
            for (int i = 123; i <= 987; i++)
            {
                ...
                rn[1] = (i - rn[0] * 100) / 10;
                rn[2] = (i % 100) % 10;
                if (rn[1] != rn[0] && rn[2] != rn[1] && rn[0] != rn[2])
                {
                    bool flag = true;
                    for (int j = 0; j < 3; j++)
                    {
                    ...
                    //num과 i의 s 갯수가 일치할 경우 arr[i]를 true로 바꿔줌
                    //s갯수가 일치한다 -> 그 위치에 숫자가 존재한다.
                    ...
                    //arr[i]가 true일때
                    //ball의 갯수가 일치하지 않을 때 true를 false로 바꿈
                    //b의 갯수가 일치한다 -> 자리는 다르지만 숫자가 존재
                }
            }
        }
        else
